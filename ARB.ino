...
void FillDACbuffer(void)
{
   ...
   for (i = PPP; i < PPP * 2; i++)
  {
    for (j = 0; j < CHANS; j++)
    {
      b[i * CHANS + j] = Waveform[j][PPP-1];
    }
  }
  return; 
  
  // Replicate the period to fill the full buffer
  for (i = 1; i < NP; i++)
  {
    for (j = 0; j < PPP * CHANS; j++)
    {
      b[j + (PPP * CHANS * i)] = b[j];
    }
  }
...
void SetWaveformType(int wft)
{
  ...
  switch (wft)
  {
    case TWI_WAVEFORM_SIN:
      ...
      { 
         if(ARBparms.Direction)
            for (i = 0; i < PPP; i++)  Waveform[ch][i] = ARBchannelValue2Counts(ch, 100 * sin(((float)i / (float)PPP) * 2.0 * PI + 2.0 * PI *ch / CHANS));
         else
            for (i = 0; i < PPP; i++)  Waveform[ch][i] = ARBchannelValue2Counts(ch, 100 * sin(((float)i / (float)PPP) * 2.0 * PI - 2.0 * PI *ch / CHANS));
      }
      break;
    case TWI_WAVEFORM_RAMP:
      ...
      { 
         if(!ARBparms.Direction)
            for (i = 0; i < PPP; i++)  Waveform[ch][(i + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,-100.0 + (200.0 * (float)i) / (float)(PPP-1));
         else
            for (i = 0; i < PPP; i++)  Waveform[ch][(i + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,-100.0 + (200.0 * (float)i) / (float)(PPP-1));
      }
      break;
    case TWI_WAVEFORM_TRI:
      ...
      { 
         if(!ARBparms.Direction)
         {
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,-100.0 + (200.0 * (float)i) / (float)(PPP/2-1));
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + PPP / 2 + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch, 100.0 - (200.0 * (float)i) / (float)(PPP/2-1));
         }
         else
         {
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,-100.0 + (200.0 * (float)i) / (float)(PPP/2-1));
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + PPP / 2 + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch, 100.0 - (200.0 * (float)i) / (float)(PPP/2-1));
         }
      }
      break;
    case TWI_WAVEFORM_PULSE:
      ...
      { 
         if(!ARBparms.Direction)
         {
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,100);
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + PPP / 2 + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,-100);
         }
         else
         {
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch, 100.0);
            for (i = 0; i < PPP / 2; i++) Waveform[ch][(i + PPP / 2 + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch, -100);
         }
      }
      break;
    case TWI_WAVEFORM_ARB:
      ...
      { 
         if(!ARBparms.Direction)
            for (i = 0; i < PPP; i++)  Waveform[ch][(i + ch * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,ARBwaveform[i]);
         else
            for (i = 0; i < PPP; i++)  Waveform[ch][(i + (CHANS -1 - ch) * PPP/(CHANS)) % PPP] = ARBchannelValue2Counts(ch,ARBwaveform[i]);
      }
      break;
    default:
      break;
...
int SetFrequency(int freq)
{
  ...
  if ((((actualF * PPP) > MINDMAFREQ) && (ARBparms.Mode == TWAVEmode)) || (((actualF) > MINDMAFREQ) && (ARBparms.Mode == ARBmode)))
  {
    // Here if above MINDMAFREQ so use DMA
    if(ARBparms.Mode == TWAVEmode) DMAclk.setFrequency(actualF * PPP);
  ...
  else
  {
    if(ARBparms.Mode == TWAVEmode) 
    {
      ARBclk.setFrequency(actualF * PPP);
    }
    if(ARBparms.Mode == ARBmode) 
    {
...
void SetEnable(bool NoTrigger = false)
{
  ...
  if ((((actualF * PPP) > MINDMAFREQ) && (ARBparms.Mode == TWAVEmode)) || (((actualF) > MINDMAFREQ) && (ARBparms.Mode == ARBmode)))
  {
    ...
    if(ARBparms.Mode == TWAVEmode) DMAbuffer2DAC((uint32_t *)0x60000000, buffer, PPP * NP * CHANS / 4,NoTrigger);
...
void SendInt24(int ival)
{
  ...
  b = (uint8_t *)&ival;
  // Send the integer to the ARB module
  sb.write(b[0]);
  sb.write(b[1]);
  sb.write(b[2]);
}
...
void ARBsyncISR(void)
{
  ...
  if(ARBparms.Mode == TWAVEmode)
  {
    ...
    if ((ARBparms.ActualFreq * PPP) > MINDMAFREQ) DMArestart();
  }
  else
  {
...
{
   if(((ARBparms.Mode == TWAVEmode) && (freq >= 0) && (freq <= 45000)) || ((ARBparms.Mode == ARBmode) && (freq >= 0) && (freq <= 1500000)))
   {
